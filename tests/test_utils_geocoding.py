...
class TestNormalizeCountryName:
    ...
    def test_alpha2_us(self):
        from shared.geocoding import PYCOUNTRY_AVAILABLE, normalize_country_name
    ...
    def test_empty_returns_none(self):
        from shared.geocoding import normalize_country_name
...
class TestNormalizeUsState:
    ...
    def test_abbr_wa(self):
        from shared.geocoding import US_AVAILABLE, normalize_us_state
        if not US_AVAILABLE:
    ...
    def test_empty_returns_none(self):
        from shared.geocoding import normalize_us_state
...
class TestIsCountryName:
    ...
    def test_none_returns_false(self):
        from shared.geocoding import is_country_name
    ...
    def test_known_country_with_pycountry(self):
        from shared.geocoding import PYCOUNTRY_AVAILABLE, is_country_name
        if not PYCOUNTRY_AVAILABLE:
            pytest.skip("pycountry not installed")
        assert is_country_name("Sweden") is True

    def test_us_state_not_country(self):
        from shared.geocoding import US_AVAILABLE, is_country_name
        if not US_AVAILABLE:
            pytest.skip("us library not installed")
    ...
    def test_two_char_without_libraries(self):
        from shared.geocoding import PYCOUNTRY_AVAILABLE, US_AVAILABLE, is_country_name
...
class TestIsUsState:
    ...
    def test_none_returns_false(self):
        from shared.geocoding import is_us_state
    ...
    def test_wa_abbr_is_state(self):
        from shared.geocoding import US_AVAILABLE, is_us_state
        if not US_AVAILABLE:
            pytest.skip("us library not installed")
...
class TestParseLocationString:
    ...
    def test_no_comma_returns_city_only(self):
        from shared.geocoding import parse_location_string
    ...
    def test_city_state_abbr(self):
        from shared.geocoding import US_AVAILABLE, parse_location_string
        if not US_AVAILABLE:
            pytest.skip("us library not installed")
    ...
    def test_city_country_full_name(self):
        from shared.geocoding import PYCOUNTRY_AVAILABLE, parse_location_string
        if not PYCOUNTRY_AVAILABLE:
            pytest.skip("pycountry not installed")
...
class TestRateLimitedNominatimGeocode:
    ...
    async def test_no_rate_limiter_calls_geocoder_directly(self):
        from shared.geocoding import rate_limited_nominatim_geocode
        bot = Mock(spec=[])  # no nominatim_rate_limiter attr
        mock_loc = _make_location()
        mock_geocoder = Mock()
        mock_geocoder.geocode = Mock(return_value=mock_loc)
        with patch("shared.geocoding.get_nominatim_geocoder", return_value=mock_geocoder):
...
class TestRateLimitedNominatimGeocodeSync:
    ...
    def test_no_rate_limiter_calls_geocoder_directly(self):
        from shared.geocoding import rate_limited_nominatim_geocode_sync
        bot = Mock(spec=[])
        mock_loc = _make_location()
        mock_geocoder = Mock()
        mock_geocoder.geocode = Mock(return_value=mock_loc)
...
class TestGeocodeZipcodeAsync:
    ...
    async def test_cache_hit_returns_coords(self, mock_bot):
        from shared.geocoding import geocode_zipcode
    ...
    async def test_cache_miss_nominatim_hit(self, mock_bot):
        ...
        with patch("shared.geocoding.rate_limited_nominatim_geocode", new=AsyncMock(return_value=mock_loc)):
            lat, lon = await geocode_zipcode(mock_bot, "98101")
        assert lat == 47.6
        assert lon == -122.3
    ...
    async def test_cache_miss_nominatim_none(self, mock_bot):
        from shared.geocoding import geocode_zipcode
        with patch("shared.geocoding.rate_limited_nominatim_geocode", new=AsyncMock(return_value=None)):
    ...
    async def test_explicit_default_country(self, mock_bot):
        ...
        with patch("shared.geocoding.rate_limited_nominatim_geocode", new=AsyncMock(return_value=mock_loc)) as m:
...
class TestGeocodeZipcodeSync:
    ...
    def test_cache_hit(self, mock_bot):
        from shared.geocoding import geocode_zipcode_sync
        mock_bot.db_manager.get_cached_geocoding = Mock(return_value=(47.6, -122.3))
    ...
    def test_cache_miss_nominatim_hit(self, mock_bot):
        from shared.geocoding import geocode_zipcode_sync
        mock_loc = _make_location(48.8, 2.3)
        with patch("shared.geocoding.rate_limited_nominatim_geocode_sync", return_value=mock_loc):
            lat, lon = geocode_zipcode_sync(mock_bot, "75001", default_country="FR")
        assert lat == 48.8

    def test_nominatim_returns_none(self, mock_bot):
        from shared.geocoding import geocode_zipcode_sync
        with patch("shared.geocoding.rate_limited_nominatim_geocode_sync", return_value=None):
...
class TestGeocodeCityAsync:
    ...
    async def test_city_with_country_nominatim(self, mock_bot):
        ...
        with patch("shared.geocoding.rate_limited_nominatim_geocode", new=AsyncMock(return_value=mock_loc)):
            with patch("shared.geocoding.rate_limited_nominatim_reverse", new=AsyncMock(return_value=None)):
...
class TestGeocodeCitySync:
    ...
    def test_exception_returns_none_tuple(self, mock_bot):
        from shared.geocoding import geocode_city_sync
        mock_bot.db_manager.get_cached_geocoding = Mock(side_effect=RuntimeError("fail"))
    ...
    def test_city_with_country_nominatim(self, mock_bot):
        ...
        mock_loc = _make_location(59.33, 18.07)
        with patch("shared.geocoding.rate_limited_nominatim_geocode_sync", return_value=mock_loc):
            with patch("shared.geocoding.rate_limited_nominatim_reverse_sync", return_value=None):
