...
answering.  Alerts go to a mesh channel, to DMs and/or to Discord/Telegram
(via the shared ``discord_webhook_urls`` / ``telegram_chat_ids`` keys).

Config section: ``[RepeaterTelemetry_Service]`` (see docs/repeater-telemetry.md).
"""
...
def parse_targets(raw: str) -> list[RepeaterTarget]:
    ...
    The password is everything after the *first* colon, so it may itself
    contain colons.  Entries without a colon use an empty (guest) password.
    """
    ...
    for entry in (raw or "").split(","):
        ...
        label = label.strip()
        if label:
            targets.append(RepeaterTarget(label=label, password=password.strip() if sep else ""))
    return targets
...
def next_level(previous: str, bat_mv: int, warn_mv: int, critical_mv: int, hysteresis_mv: int) -> str:
    ...
    if _LEVEL_RANK[raw] >= _LEVEL_RANK[previous]:
        return raw
    # Improving: require the hysteresis margin above the relevant threshold
...
class RepeaterTelemetryService(BaseServicePlugin):
    ...
    description = "Polls repeater status (battery, uptime, RSSI) and warns on low battery"
    ...
    settings_schema = [
        {
            ...
            "type": "password",
            "default": "",
            "help": "Comma-separated name:password (or key-prefix:password). "
                    "Password may be the guest password; omit ':password' for none.",
            "required": True,
        },
        ...
        {"key": "alert_channel", "label": "Alert channel", "type": "str", "default": "",
         "help": "Mesh channel for alerts (e.g. #admin). Empty = no channel alerts."},
        {"key": "alert_dm", "label": "Alert DMs", "type": "list", "default": "",
         "help": "Comma-separated contact names that get alerts as DM."},
    ...
    def _cfg(self, key: str, fallback: str = "") -> str:
        return (self.bot.config.get(self.config_section, key, fallback=fallback) or "").strip()

    def _cfg_int(self, key: str, fallback: int) -> int:
        try:
            return int(float(self._cfg(key, str(fallback))))
        except ValueError:
            self.logger.warning(f"[RepeaterTelemetry] invalid {key}, using {fallback}")
            return fallback

    def _cfg_float(self, key: str, fallback: float) -> float:
        try:
            return float(self._cfg(key, str(fallback)))
        except ValueError:
            self.logger.warning(f"[RepeaterTelemetry] invalid {key}, using {fallback}")
            return fallback

    def _load_settings(self) -> None:
        """(Re-)read settings; called every cycle so config reloads apply."""
        self.targets = parse_targets(self._cfg("repeaters"))
        self.interval_s = max(5, self._cfg_int("interval_minutes", 60)) * 60
        self.delay_between_s = max(5, self._cfg_int("delay_between_seconds", 30))
        self.request_timeout_s = max(0, self._cfg_int("request_timeout_seconds", 0))
        self.warn_mv = self._cfg_int("warn_mv", 3500)
        self.critical_mv = self._cfg_int("critical_mv", 3300)
        self.hysteresis_mv = max(0, self._cfg_int("hysteresis_mv", 100))
        self.repeat_alert_s = max(0.0, self._cfg_float("repeat_alert_hours", 12)) * 3600
        self.offline_after = max(0, self._cfg_int("offline_after_failures", 3))
        self.alert_channel = self._cfg("alert_channel")
        self.alert_dm = [n.strip() for n in self._cfg("alert_dm").split(",") if n.strip()]
        self.retention_days = max(1, self._cfg_int("retention_days", 90))
        self.silence_mesh = self._cfg("silence_mesh_output", "false").lower() in ("1", "true", "yes", "on")
        self.initial_delay_s = max(0, self._cfg_int("initial_delay_seconds", 120))
        self.templates = {k: self._cfg(f"template_{k}", v) or v for k, v in DEFAULT_TEMPLATES.items()}
    ...
    async def start(self) -> None:
        self._init_db()
        if not self.targets:
            self.logger.warning("[RepeaterTelemetry] no repeaters configured (repeaters = name:password, ...)")
        self._stop_event.clear()
        self._task = asyncio.create_task(self._run_loop())
        self._running = True
        self.logger.info(
            f"[RepeaterTelemetry] started: {len(self.targets)} repeater(s), every {self.interval_s // 60} min, "
            f"warn < {self.warn_mv} mV, critical < {self.critical_mv} mV"
        )
    ...
    async def _run_loop(self) -> None:
        if await self._sleep(self.initial_delay_s):
            return
        while not self._stop_event.is_set():
            ...
            if await self._sleep(self.interval_s):
                return

    async def _sleep(self, seconds: float) -> bool:
        """Sleep until timeout, stop or poll-now. Returns True when stopping."""
        self._poll_now.clear()
        stop_wait = asyncio.create_task(self._stop_event.wait())
        poll_wait = asyncio.create_task(self._poll_now.wait())
        try:
            await asyncio.wait({stop_wait, poll_wait}, timeout=seconds, return_when=asyncio.FIRST_COMPLETED)
        finally:
            stop_wait.cancel()
            poll_wait.cancel()
    ...
    def _find_contact(self, label: str) -> Optional[dict[str, Any]]:
        ...
        if contact is None and all(c in "0123456789abcdefABCDEF" for c in label) and len(label) >= 4:
    ...
    async def _evaluate(self, r: PollResult) -> None:
        ...
        if r.bat_mv is not None:
            ...
            if level != prev:
                if level == LEVEL_OK:
                    messages.append(self._fmt("recovered", r.name, r.bat_mv, 0))
                else:
                    # Also fires on critical -> warn, so everyone sees it improved
                    messages.append(self._fmt(level, r.name, r.bat_mv, 0))
                st["level"] = level
            else:
                self._maybe_repeat(st, now, messages, r)
        await self._finish(st, now, messages)
    ...
    async def send_alert(self, text: str) -> None:
        ...
        if not self.silence_mesh:
            ...
            for recipient in self.alert_dm:
                try:
                    await self.bot.bot_tx_rate_limiter.wait_for_tx()
                    await self.bot.command_manager.send_dm(recipient, text, skip_user_rate_limit=True)
                except Exception as e:
                    self.logger.error(f"[RepeaterTelemetry] DM alert to {recipient} failed: {e}")
